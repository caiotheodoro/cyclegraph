...
FRAME_SIGNAL: Fixture = {
    ...
    "fps_sampled": 4.0,
    "n_frames": 4,
    "manipulation": [True, True, False, None],
    "hands_visible": [2, 2, 1, None],
    "hand_box_width_px": [212.0, 208.5, None, None],
...
FRAME_SIGNAL_BROKEN: list[Broken] = [
    ...
    ("series lengths disagree", _with(FRAME_SIGNAL, hands_visible=[2, 2, 1])),
    ("manipulation null without hands_visible null", _with(FRAME_SIGNAL, hands_visible=[2, 2, 1, 0])),
...
HAND_SPEED_BROKEN: list[Broken] = [
    ("zero speed for a failed flow", _with(HAND_SPEED, n_flow_null=200, flow_null_rate=200 / 690, rms_speed_mm_s=0.0)),
    ("ok below the coverage floor", _with(HAND_SPEED, n_with_box=300, n_flow_null=3, coverage=300 / 750, flow_null_rate=3 / 300)),
...
HAL_AKKAS: Fixture = {
    ...
    "scale_rev": "acgih-2001-table/akkas-2015-fit",
    "duty_cycle": 0.68,
    "hz": None,
    "rms_speed_mm_s": 612.4,
    "out_of_range": False,
...
HAL_RADWIN: Fixture = _with(
    HAL_AKKAS, mapping="radwin-2015-freq-dc", scale_rev="acgih-2001-table/radwin-2015-fit",
    hz=0.42, rms_speed_mm_s=None, hal=4.6,
)
...
HAL_BROKEN: list[Broken] = [
    ...
    ("empty scale_rev", _with(HAL_AKKAS, scale_rev="")),
    ("hal off the scale", _with(HAL_AKKAS, hal=11.0)),
]
...
AGGREGATE: Fixture = {
    ...
    "weighting": "clip",
    "design_effect": 1.44,
    "design_effect_width_ratio": 1.2,
    "design_effect_mc_band": 0.05,
    "bootstrap_b": 10000,
    "seed": 777,
    "iid_ci95_for_contrast": [4.02, 4.20],
...
AGGREGATE_STRATUM: Fixture = _with(
    ...
    n_clips=12000, n_workers=700, n_factories=28, k_factories=28, k_workers=700,
    max_factory_share=0.11, weighting="worker",
)

AGGREGATE_BROKEN: list[Broken] = [
    ("factory_id field", _with(AGGREGATE, factory_id="factory_007")),
    ...
    ("stratum below the factory floor", _with(AGGREGATE_STRATUM, n_factories=4, k_factories=4)),
    ("stratum below the worker floor", _with(AGGREGATE_STRATUM, n_workers=49, k_workers=49)),
    ("one factory dominates", _with(AGGREGATE_STRATUM, max_factory_share=0.55)),
    ...
    ("k does not equal n", _with(AGGREGATE, k_factories=80)),
]
...
CLAIM: Fixture = {
    ...
    "status": "UNTESTED",
    "source": "results/h1_duty_cycle.json",
    "interval": None,
}
...
CARD_BROKEN: list[Broken] = [
    ...
    ("verified with an open gap", _with(CARD, verdict="VERIFIED", claims=[_with(CLAIM, status="HOLDS")])),
    ("source outside results/", _with(CARD, claims=[_with(CLAIM, source="docs/BENCHMARK.md")])),
    ("identifier in a statement", _with(CARD, claims=[_with(CLAIM, statement="factory_003 is high")])),
    ("duplicate claim ids", _with(CARD, claims=[CLAIM, CLAIM])),
    ("unknown status", _with(CARD, claims=[_with(CLAIM, status="PASSED")])),
]
...
VALID: dict[str, list[Fixture]] = {
    ...
    "HandSpeedEstimate": [HAND_SPEED, HAND_SPEED_LOW_COVERAGE],
    "HALScore": [HAL_AKKAS, HAL_RADWIN],
