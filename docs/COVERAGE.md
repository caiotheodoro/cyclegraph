...
| **Which source frame a 4 Hz instant means** | **Known to one frame, 33 ms.** The box and label at an instant come from a `-vf fps=4` decode whose frame selection is ffmpeg's; the flow pair is located by index from a native-rate decode. The two can differ by one source frame — about 21 px of hand travel at 600 mm/s against a ~96 px box, so the mask can sit partly off the hand. Removing it would need one decode to serve both, which the detector and the speed path do not share. `scripts/build_signal.py` (`stream_pairs`). |
| **Hand speed above ~900 mm/s** | **Under-measured, in the flattering direction, by a factor approaching five.** Dense optical flow smooths across the hand/background depth discontinuity once the hand's displacement exceeds its window, and reports the background's motion inside the hand box — which the rubric then subtracts. Measured for Farneback and RAFT-small, at four decode resolutions and five parameter settings; none holds (`docs/DECISIONS.md` D044, D050). The Akkas equation is fitted over **255.3–1288.0 mm/s** (Table 1, as `src/cyclegraph/exposure/hal.py` records it), so most of its upper half is where this bites — a larger portion than D050 first said (D055). Not a bug to fix: it is a bound on what a dense estimator can say. |
| **How far a mask's hand box sits from the detector box the rubric was written against** | **Not measurable, and it scales every speed linearly.** 100DOH's weights are unobtainable (`docs/DECISIONS.md` D037), so the two conventions cannot be run on the same frames and the offset cannot be measured by anyone. A *x*% offset is arithmetically the same as an *x*% change in `hand_breadth_mm`, so `docs/BENCHMARK.md`'s 79.5/90.4 mm rows already report the sensitivity to an offset of about ±6%. Whether the real offset is larger than that is unknown. D047. |
| **Is the pipeline reproducible with the detector it pre-registered** | **No, and this is not a task.** `docs/REPRODUCTION.md` promises a stranger commensurable numbers; the primary detector cannot be downloaded from either link its authors published, and no mirror exists. D037. |
